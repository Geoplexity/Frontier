 ...
 */
   ...
   public class SKCircleShape extends SKBaseShape
   {
      ...
      public SKCircleShape(SKMainFrame frame, int newID, int x, int y)
      {
         ...
         frameMain=frame;
      //Create points
         if(!(this instanceof SKCircleSubShape))
   ...
   //Should this shape be drawn?
      ...
      {//System.out.println("entered circle paint special");
      ...
      {
      //int radius = radial.getX()-center.getX();
      //ellipseShape.setFrameFromCenter(center.getShapeX(),center.getShapeY(),2*(radial.getX()-center.getX()),2*(radial.getY()-center.getY()));
         ellipseShape.setFrameFromCenter(center.getX()+(center.getWidth()/2),center.getY()+(center.getHeight()/2),center.getX()+(center.getWidth()/2)+20,center.getY()+(center.getHeight()/2)+20);
         return ellipseShape.intersects(x-2,y-2,4,4);
      }
      public void increaseRadius( int x, int y)
      {bifur = true;
         //radius = radius + java.lang.Math.sqrt(java.lang.Math.pow((double)x,2)+java.lang.Math.pow((double)y,2));
         radius = distance(center.getShapeX(), center.getShapeY(), x, y);
         radius = radius/(40*frameMain.lastScale);
      	// java.lang.Math.sqrt(java.lang.Math.pow(((double)center.getX()-x),2)+java.lang.Math.pow(((double)center.getY()-y),2));
         repaint();
      }
      ...
      // (int)ellipseShape.getmx-1,(int)ellipseShape.y-1,
