...
//   outf.open("explicitChild.out", ios::app);
   ...
   for(i=1;i<=numOfTree;i++)
   {
      ...
      {
          outf << "The upper one is what we need" << endl;
...
int getClusterType(Graph &graph0, Cluster &theCluster)
{
	...
	outf<<"Edge Name: "<<tempEdge.returnName()<<endl;
	outf<<"Edge Code: "<<getEdgeCodeDRPlanner(tempEdge,theCluster)<<endl;
	outf<<"Edge Type: "<<tempEdge.returnType()<<endl;
    ...
    outf<<"Getting Cluster "<<theCluster.returnName()<<" Type: "<<endl;
    outf<<"LSCount:     "<<LSCount<<endl;
    outf<<"distCount:   "<<distCount<<endl;
    outf<<"pointCount:  "<<pointCount<<endl;
    outf<<"totalCount:  "<<totalCount<<endl;
    outf<<"notIncid:    "<<notIncidCount<<endl;

    if(LSCount==0 && distCount>1)
    {
	outf<<"Type set to 0"<<endl;
	return 0;
    }
...
/*	for(int i=1;i<=vLen; i++)
	...
	outf << "has distance? " << hasDistance << endl;
	outf << "inList1========" << endl;
	...
	outf << "final inList1========" << endl;
	for(int i =1; i<=inList1.returnLen(); i++)
		outf << inList1.retrieve(i) << "    ";
	outf << endl;
	outf << "final inList2========" << endl;
	for(int i =1; i<=inList2.returnLen(); i++)
		outf << inList2.retrieve(i) << "    ";
	outf << endl;
...
//	outf.open("canCombine.out", ios::app);
	...
	outf<<"lenOverlap = " << lenOverlap<< endl;
	//no overlapped vertices, return false
	if(lenOverlap <= 0) 
	{	outf<< "lenoverlap <=0, return false" << endl;
		return false;
	}
	if((specialCluster(graph0, c) || specialCluster(graph0, cl0)) 
	    && lenOverlap >= 2)
	{
		outf <<"c is specialcluster?: " << specialCluster(graph0, c) <<endl;
		outf <<"cl0 is specialcluster?: " << specialCluster(graph0, cl0) <<endl;
		return true;
	}
	...
	{
		outf << "the cluster type is not 2" << endl;
		return true;
	}
	else
	{
		...
		{	outf<<"cl0 is type " << getClusterType(graph0, cl0) <<endl;
			outf<<"c is type " << getClusterType(graph0, c) <<endl;
			outf << "return true:" << endl;
			return true;
		}
	} 
...
//	outf.open("addchild.out", ios::app);
   ...
   implicitChild(newTree, nameList, popedTrees, F, graph0);
	outf << "after implicitchild " << endl;
	newTree.output(outf);

   outf << "implicitChild with popedTrees" << endl;
   printForest(DR_Trees, cout, 0);
...
//	outf.open("setAlias.out", ios::app);
   ...
   for(i=1;i<=length;i++)
		...
		||graph0.returnVertByName(theCluster.returnOrigV(i)).returnType()==3)
	{
		...
		outf<<"Alias found: "<<alias<<endl;
		break;
	}

   if(alias==-1)
   {
	outf<<"Error: No Alias found."<<endl;
	return;
   }
   ...
   for(i=1;i<=length;i++)
   {
	currVertID=theCluster.returnOrigV(i);
        ...
        if(currVertType==7 || currVertType==3)
        {
                ...
                outf<<"Line appended: "<<currVertID<<endl;
                continue;
        }
	...
	outf<<"Checking Edge: "<<currEdge<<endl;
	...
	if(aliasPart==1)
	{
		...
		outf<<"Left appended: "<<currVertID<<endl;
	}

	if(aliasPart==2)
	{
		theCluster.right.append(currVertID);
		outf<<"Right appended: "<<currVertID<<endl;
	}
   }
}
...
//	outf.open("pseudoMinDense.out", ios::app);
   ...
   outf<<"IN PSEUDOMINDENSE"<<endl << "The INPUT cluster is:" << endl;
   theCluster.output(outf);
	...
	isChange=false;
   	...
   	for(i=1;i<=length;i++)
		for(j=i+1;j<=length;j++)
		{
			...
			outf<<"Cluster 1: "<<theCluster.children.retrieve(i).returnName()<<endl;
			tempCluster.children.retrieve(i).output(outf);
			outf<<"Cluster 2: "<<theCluster.children.retrieve(j).returnName()<<endl;
...
//	outf.open("canMerge.out", ios::app);
	...
	ofstream dd1("dd1.out", ios::app);
	ofstream dd2("dd2.out", ios::app);
...
void mergeCluster(Graph &graph0, int coreName, Cluster &parent, Cluster child1, Cluster child2)
{
	...
	ofstream outf;
//	outf.open("mergeCluster.out", ios::app);
	...
	outf << "after getting fronVertices, the fronts are:" << endl;
	for(int i=1; i<=fronV.returnLen(); i++)
		outf << fronV.retrieve(i) << endl;
	outf << "the orig are: " << endl;
	for(int i=1; i<=orig.returnLen(); i++)
		outf << orig.retrieve(i)  << endl;

	outf << "let's do inner v part" << endl;
	//inner vertice
	for(int i=1; i<=orig.returnLen(); i++)
	{
		int vertex = orig.retrieve(i);
		outf << "vertex is: " << vertex << endl;

		Vertex V=graph0.returnVertByName(vertex);
		outf << "corresponding vertex is: " << V << endl;
		if(!fronV.hasElem(vertex))
		{
			outf<<"fronv has it" << endl;
			for(int j=1; j<=V.numIncid(); j++)
			{
				int edgeName = V.returnIncid(j);
				outf << "edge name is: " << edgeName << endl;
				Edge edge=graph0.returnEdgeByName(edgeName);
				int end1 =edge.returnEnd1();
				int end2 = edge.returnEnd2();
				outf << "end1, end2: " << end1 << ", " << end2 << endl;
	...
	for(int i=1; i<=fronV.returnLen(); i++)
		outf << fronV.retrieve(i) << endl;
 	outf << "after getting inner vertices" << endl;
	...
	outf << "after getting outer Edges" << endl;
	//inner edges
	for(int i=1; i<=fronV.returnLen();i++)
	{
	...
	outf << "after getting inner Edges" << endl;
...
int modifyTree(Graph &graph0, Cluster &newC)
{
	...
	//ugly
	ofstream outf;
//	outf.open("modifyTree.out", ios::app);
	outf << "the total num of vertices is: " << graph0.returnNumVer() << endl;
	outf << "the total num of edges is: " <<graph0.returnNumEdg() << endl;
	if ((graph0.returnNumVer()!= 8 || graph0.returnNumEdg()!=18)
	  &&(graph0.returnNumVer()!= 11 || graph0.returnNumEdg()!=23))
		return 0;

	outf << "The begining, the cluster is: " << endl;
	newC.output(outf);
	...
	for(int i=1; i<=newC.children.returnLen();i++)
	{
		...
		for(int j=i+1; j<=newC.children.returnLen();j++)
		{
			...
			if(canMerge(graph0, child1, child2))
			{
				...
				i = 1;
				j = 2;
				outf << "end of if-canMerge " << endl;
			}
		}
	}
	
	outf << "AT THE END, the newC is: " << endl;
	newC.output(outf);
...
{
	ofstream outf;
	outf.open("setConstrain.out", ios::app);
	outf << " =============== begin le ============================" << endl;
	...
	outf << "The over list contains:" << endl;
   ...
   while(DR_Trees.returnLen()>0)
   {
	 ...
	 if(graph0.returnDimen()==3)
	 {
	  ...
	  file2<<"Before addChild "<<endl;
          newTree.output(file2);
          file2<<"right before ADDCHILD" << endl;
          printForest(DR_Trees, file2, 0);
          addChild(newTree, popedTrees, DR_Trees, F, graph0);
	  file2<<"After addchild"<<endl;
	  graph0.output(cout);
	  ...
	  cout<<"after deleting inner edges"<<endl;
          ...
          F.output(file2);
	  file2<<"right before modify DR_Tree" << endl;
          printForest(DR_Trees, file2, 0);
     ...
     if(graph0.returnDimen()==3)
     {
	  ...
	  //out<<"Get new Cluster"<<endl;
          ...
          {
		file2 << "The final cluster is: " << endl;
		Cluster finalT = DR_Trees.retrieve(1);
		finalT.output(file2);
		file2 << "The children of the final tree are: " << endl;
