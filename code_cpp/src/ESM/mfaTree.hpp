...
private:
  ...
  std::vector <Node *> Block;      // The dynamic list of nodes of the tree to be created
  std::vector <Condition *> Cases; // The dynamic list of condition for nodes of the tree to be created
...
public:
  ...
  void build_Tree(int select)
  {
    ...
    */
    ...
    if (mode!=2)     // If user inputs a number other than 1 or 2, default mode is 1
      if (mode!=1)
	{
	  std::cout << "Entered an invalid mode. So, default mode is Find mode\n";
	  mode=1;
	}
	...
	//std::cout << "String :" << buffer0 << std::endl;
	...
	*/
	...
	std::cout << "The Tree structure and constraints can be saved in a directory." << std::endl;
	std::cout << "Save (Y/N)? ";
	char save_info;
	std::cin >> save_info;
  ...
  void copyTreeIntoArray(int *array, char *strArray)
  {
	...
	*/
	if (array[i]!=-2)
	  {
	    std::cout << "index=" << i << " " << array[i] << std::endl; //<< " " << name[i] << std::endl;
	  }
	else
	  break;
      }
  ...
  //pops the first element out of the std::vector
  ...
  //pops the first element out of the std::vector of Nodes
...
void copyArrayIntoTree(int &startI, int *&array, char *&strArray)
  ...
  */
  ...
  std::string search_Tree(Graph &graph0, Cluster &theCluster, std::ostream &inputFile, int flag)
  {
    ClustData test;
	...
	std::string str1, str2, str3, str4, str5, str6, str7, str8, str9, str10, str11, str12, str13;
	std::string rep_1, rep_2, rep_3, rep_4, rep_5, rep_6, rep_7, rep_8, rep_9, rep_10, rep_11, rep_12, rep_13;
...
*/
  ...
  std::string search_Tree(Graph &graph0, Cluster &theCluster, std::ostream &inputFile)
  {
    ClustData test;
    test=summCluster(graph0, theCluster);
	...
	outf << "vars are: " << getVarString() << std::endl;
	...
	outf << "vars are: " << getVarString() << std::endl;
      	...
      	outf << "After the xyz replacement, the tempstring is: " << std::endl << tempString << std::endl;
	int sig_flag=0;
...
*/
  ...
  void search_Tree()
  {
	...
	std::cin >> a >> b >> c >> d >> e;
	std::cout << "Search No." << no_of_searches << ": " <<a<< "," <<b<< "," <<c<< "," <<d<< "," <<e;
	Searches.resize(no_of_searches);
	t_str.resize(no_of_searches);
	Searches[no_of_searches-1]=new Ord_Set;
	Searches[no_of_searches-1]->Set_Val(a,b,c,d,e);
	...
	for (int x=0; x<no_of_searches; x++) // Search the Tree
	  {
	    ...
	    d=Searches[x]->ret_4();
	    e=Searches[x]->ret_5();
	    std::cout << "Search No." << x+1 << ": (" <<a<< "," <<b<< "," <<c<< "," <<d<< "," <<e<<  "): ";
	    std::cout << "Parent: " << t_par[x] << " " << Block[t_par[x]]->Ret_label() << std::endl;
	    //std::cout << "\nGlobal: " << glo_flag;
...
/*
  std::cout << Block[0]->Find_Eqn(3,2,3,2,1,0) << std::endl;
  std::cout << Block[0]->Find_Eqn(3,2,0,1,1,0) << std::endl;
  std::cout << Block[0]->Find_Eqn(2,1,0,1,1,0) << std::endl;
  std::cout << Block[0]->Find_Eqn(3,2,2,1,1,0) << std::endl;
  std::cout << Block[0]->Find_Eqn(4,3,3,2,1,0) << std::endl;
  std::cout << Block[0]->Find_Eqn(3,1,3,2,1,0) << std::endl;
  std::cout << Block[0]->Find_Eqn(1,0,3,2,1,0) << std::endl;
  std::cout << Block[0]->Find_Eqn(3,0,0,1,1,0) << std::endl;
  std::cout << Block[0]->Find_Eqn(3,0,5,1,1,0) << std::endl;
  std::cout << Block[0]->Find_Eqn(3,0,5,2,1,0) << std::endl;
  std::cout << Block[0]->Find_Eqn(3,2,1,0,1,0) << std::endl;
  std::cout << Block[0]->Find_Eqn(5,2,3,0,1,0) << std::endl;
  std::cout << Block[0]->Find_Eqn(3,2,3,0,1,0) << std::endl;
  std::cout << Block[0]->Find_Eqn(5,1,3,0,1,0) << std::endl;
  std::cout << Block[0]->Find_Eqn(2,1,1,0,1,0) << std::endl;
  std::cout << Block[0]->Find_Eqn(2,0,2,1,1,0) << std::endl;
  std::cout << Block[0]->Find_Eqn(2,0,5,1,1,0) << std::endl;
  std::cout << Block[0]->Find_Eqn(2,0,3,2,1,0) << std::endl;
  std::cout << Block[0]->Find_Eqn(3,0,5,2,1,0) << std::endl;
  std::cout << Block[0]->Find_Eqn(3,2,2,0,1,0) << std::endl;
  std::cout << Block[0]->Find_Eqn(5,1,2,0,1,0) << std::endl;
  std::cout << Block[0]->Find_Eqn(5,2,2,0,1,0) << std::endl;
  std::cout << Block[0]->Find_Eqn(6,4,6,5,1,0) << std::endl;
  std::cout << Block[0]->Find_Eqn(5,3,2,0,1,0) << std::endl;
  std::cout << Block[0]->Find_Eqn(3,1,1,0,1,0) << std::endl;
  std::cout << Block[0]->Find_Eqn(1,0,4,3,1,0) << std::endl;
*/
...
{
  //std::cout<<"The ordered sets are: "<<type1<<part1<<type2<<part2<<const_type<<std::endl;
