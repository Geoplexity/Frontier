...
int getClusterType(Graph &graph0, Cluster &theCluster)
{
    ...
    outf<<"Edge Code: "<<getEdgeCodeDRP(tempEdge,theCluster)<<std::endl;
    outf<<"Edge Type: "<<tempEdge.returnType()<<std::endl;
    if(getEdgeCodeDRP(tempEdge,theCluster)==0 && tempEdge.returnType()==6)
        distCount++;
    if(getEdgeCodeDRP(tempEdge,theCluster)==1 && tempEdge.returnType()!=1)
        notIncidCount++;
    }
...
//  outf.open("checkBananas.out", ios::app);
        ...
        for(i=1;i<=length;i++)
    {
        ...
        int value=getEdgeCodeDRP(theEdge,theCluster);
...
int getEdgeCodeDRP(Edge &theEdge, Cluster &theCluster)
{
  ...
  std::ofstream outf;
  //    outf.open("getEdgeCodeDRP.out", ios::app);
  outf << "The beginning:" << std::endl;
