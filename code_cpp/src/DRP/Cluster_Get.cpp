...
int extension(Graph &F, Graph &graph0, std::ostream &file1, std::ostream &file2)
{
    ...
    file2<<"Extension, nLabeled="<<nLabelF<<std::endl;
    file2<<"Contents of F:"<<std::endl;
    F.output(file2);
    if(nLabelF==0)
    {
      file2<<"nLabeled=0"<<std::endl;
      file2<<"Extension returns 0"<<std::endl;
      file2<<"----------------end----------------"<<std::endl;
      return 0;
    }
  ...
  {
    file2 << "the v0name is: " << v0Name << std::endl;
     possible=0;
     if(F.hasVert(v0Name))
     {
         v0=F.returnVertByName(v0Name);
         if(v0.returnLabel()==0&&(!v0.isFrozen())) possible=1; //v0 in F, but NOT in new cluster
     file2 << "v0 in F is: " << v0 << std::endl;
     file2 << "possible: " << possible << std::endl;
     }
     ...
     file2 << "v0 in graph0 is: " << v0 << std::endl;
     file2 << "possible: " << possible << std::endl;
     }
    ...
    notIncidCount=0;
        ...
        for(j=1;j<=nVertF;j++)           // for every vertex v1 in F
        {
         ...
         if(edge.returnType()!=1) notIncidCount++;
                 ...
                 file2<<"Update:  extEdgeNum="<<extEdgeNum<<" eWeight="<<eWeight<<std::endl;
              }
           }
        }

    file2<<"Final:  extEdgeNum="<<extEdgeNum<<" eWeight="<<eWeight<<std::endl;

    bool test;

    std::ofstream output;

    output.open("../MFA/textSpecCase.txt", std::ofstream::app);
    ...
    file2<<"Check Special Case: "<<test<<std::endl;

        if(test) output<<"SPECIAL CASE = TRUE"<<std::endl;
        else output<<"SPECIAL CASE = FALSE"<<std::endl;

    file2<<"Dimension="<<F.returnDimen()<<std::endl;
...
//        F.distribute0(edge,file2);
           ...
           F.output(file2);
           file2<<"Extension returns 1"<<std::endl;
           file2<<"----------------end----------------"<<std::endl;
           return 1;
...
Cluster * getCluster(Graph &F, Graph &graph0)
{
    ...
    std::ofstream outf("getCluster.out", std::ios::app);
    outf<<"Getting Cluster "<<nextVerName<<std::endl;
    outf<<"Contents of F before new labeling"<<std::endl;
    F.output(outf);
    ...
    frontiers.makeEmpty();
    //out<<"before getCluster"<<std::endl;
    //F.output(out);
    ...
    for(i=1;i<=nVerF;i++) // for every vertex in F
    {
        ...
        if(vOld.returnLabel()!=0 && vOld.isFrozen()==0)  // make sure vOld in new cluster
        {
           ...
           outf<<"after append originalV"<<std::endl;
           F.output(outf);
    ...
    /***
    outf<<"frontiers"<<std::endl;
    for(i=1;i<=frontiers.returnLen();i++)
       out<<frontiers.retrieve(i)<<std::endl;
    out<<"innerVers"<<std::endl;
    for(i=1;i<=innerVers.returnLen();i++)
       out<<innerVers.retrieve(i)<<std::endl;
    ***/
    ...
    outf << "coreWeight is" << coreWeight << std::endl;
       //get outer edges
       eWeight=0;
       for(j=1;j<=nVerF;j++)
       {
        ...
        eOld=F.returnEdgeByName(eOld.returnName());
          ...
          if(v3.returnScan()!=-1&&eOld.returnName())//v3 is not an inner vertex
          {
             ...
             if(v3.returnLabel()!=0&&v1Name!=v3Name&&!outerE.hasElem(eOld))
             {
                ...
                outf << "eWeight is: " << eWeight << std::endl;
             }
          }
       }
    ...
    outf << "coreWeight is" << coreWeight << std::endl;
       ...
       //form inner edges
       eWeight=0;
       outf<<"Calculating Inner Edges"<<std::endl;
    ...
    //out<<"new Cluster"<<std::endl;
    //new_cluster->output(out);
    //out<<"after getCluster F is"<<std::endl;
    //F.output(out);

    outf << "The final cluster is: " << std::endl;
    (* new_cluster).output(outf);
    outf<<"END GETCLUSTER"<<std::endl;
