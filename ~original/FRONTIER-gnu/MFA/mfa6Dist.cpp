...
int removeFlow(Cluster &C, Graph &F, Graph &graph0)
{
	...
	ofstream outf("removeFlow.out", ios::app);
	
	outf << "AT BEGINNING +++++++++++++++" << endl << "The F is: " << endl;
	F.output(outf);
	...
	for(int i=1; i<=C.returnFronLen(); i++)
	{
		...
		outf << "the frontier vertex is: " << *fv << endl;
		...
		for(int j=1; j<=incid.returnLen(); j++)
		{
			...
			if(ep!=NULL)
			{
				outf << "The choosed edge is: " << *ep << endl;
...
int pushOutside(Cluster &C, Graph &F, Graph &graph0, ostream &file1, ostream &file2)
{
   ...
   for(i=1;i<=C.returnOuterELen();i++)
   {
	...
	if(targetEdgeTemp.returnName()==0) continue;
        file2<<"Testing Outer Edge: "<<targetEdgeTemp<<endl;
	...
	file2<<"Total Flow: "<<totalFlow<<endl;
   ...
   file2<<"OUTER DISTRIBUTE END"<<endl;

   K=copyG(F, CF);    // copy from F to CF

   file2<<"INNER DISTRIBUTE"<<endl;

   //distribute inner edges
   file2 << "Look at the results of distributing inner edges" << endl;
   for(i=1;i<=nFront;i++)
   {
      F.distribute0(C.returnInnerE().retrieve(i), file2);
   }

   file2<<"INNER DISTRIBUTE END"<<endl;
...
// NEW FILLER SECTION
   ...
   for(i=1;i<=nInner;i++)
   {
		...
		file2 << "remove edge is: " << rmEdgeName << endl;
        	//restore i to distribute the inner edge again.
        	i--;
        }
      	file2 <<"The inner result is: " <<  a << endl;
   }
	 ...
	 if(C.returnOuterE().hasElem(targetEdge)) continue;
            ...
            &&(!v2.isFrozen()))
         {
		  if(targetEdge.returnFlow1()+targetEdge.returnFlow2()==0)
		  {
			...
			file2<<"Distribute Edge in F"<<endl;
			...
			file2<<"Distribute Edge in CF"<<endl;
		  ...
		  file2<<"type value read in pushOutside="<<type<<endl;
  		  if(type==2)
		  {
