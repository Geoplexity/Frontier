...
bool canCombine (Cluster &c, Cluster &cl0, Graph &F, Graph &graph0)  // subset operator
{
	...
	int lenOver = overlappedVerts.returnLen();
	for(i = 1; i <= lenOver; i++)
	...
	if(F.returnDimen() == 2)
	{
		if(lenOver < 2) return false;
		return true;
	}
	
	//3D
	//less than 3 objects, return false
	if(lenOver < 3) return false;
	...
	if(getClusterType(graph1, DR_Trees.retrieve(DR_Trees.returnLen())) != 2) return true;
	for(i = 1; i <= nOrig; i++)
	{
		for(int j = 1; j <= nOrig0; j++)
		{	//exists the independent edge
			if(graph0.returnEdgeByEnds(i, j).returnName() != 0
				 && graph1.returnEdgeByEnds(i, j).returnName() == 0)
				return true;
		}
	}
	return false;
	//	int nFront = frontiers.returnLen();	
	//
...
int checkClusterForBananas(Graph &graph0, Cluster &theCluster)
{
        ...
        for(i=1;i<=length;i++)
	{
		...
		cout<<"typeValue="<<typeValue<<endl;
		cout<<"Edgebetween="<<(edgeBetween ? "true" : "false")<<endl;

	}

	cout<<"Final Edgebetween="<<(edgeBetween ? "true" : "false")<<endl;
	...
	cout<<"Over1: ";
	for(i=1;i<=over1.returnLen();i++)
		cout<<over1.retrieve(i);
	cout<<endl;
	cout<<"Over2: "; 
        for(i=1;i<=over2.returnLen();i++)
                cout<<over2.retrieve(i);
        cout<<endl;
