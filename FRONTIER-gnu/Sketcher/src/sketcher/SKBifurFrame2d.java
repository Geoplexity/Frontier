/**
 * Title:        SKBifurFrame2D
 * Description:  Frame used to display bifurcations in two dimensions
 Currently working on the communication part!
 */
   package sketcher;
   import javax.swing.*;
   import java.awt.*;
   ...
   public class SKBifurFrame2d extends JDialog
   {
      ...
      public int rePickID = -1; // Defined a repick ID

      public SKBifurcationArray2d bifurcations;
      ...
      public JButton btnRepick = new JButton(); // Button added for repick
      public JDialog reSolveDialog =null;
      public JDialog rePickDialog = null; // Dialogue added separately for resolve and repick!!!
      ...
      public static SKGTNArray Groups = new SKGTNArray(100);//Added for tree
      public static int ID,w,h;
      public static SKScrollableTree panel= null;
      public static int[] flag;
      TreeDialog treeClusters;
      SKScrollableTree pnlTree;
      public static SKText mouseOverItem = null;
      public static SKText clusterName = null;
      ...
      public SKBifurFrame2d(Frame owner, String title, boolean modal,SKGTNArray allGroups, int actID, int[] fin)
      {
	 ...
	 //panel = new SKScrollableTree(this,frame1,Groups,null,actID,fin);
      ...
      private void jbInit() throws Exception
      {
         ...
         btnRepick.setText("Re-Pick"); // Set the action routines for the repick button!
         btnRepick.addActionListener(
	 ...
	 pnlConfirm.add(btnRepick, null); // Added the repick button.
	 ...
	 System.out.println("111 shapes size = "+Shapes.size());
	 cloneConstraints(Shapes);*/
         for(int i=0; i<pnlShapes.getComponentCount();i++)
         {
            ...
            if(!(try1 instanceof SKPointSubShape))
            {
               ...
               if(frameMain.solvingShapes.indexOf(try2) == -1)
                  frameMain.solvingShapes.add(try2);
            }
         }
         repaint();
         frameMain.repaint();
	 ...
	 //JPanel tree = new JPanel();  /// panel for the tree
	 ...
	 sbStatus.setPreferredSize(new Dimension(20, 15));
         ...
         sbStatus.addPanel("No Cluster Selected",500,sbStatus.ALIGN_CENTER);
	 pnlTree = new SKScrollableTree(this, frameMain,Groups,null,ID,flag,1); // changed the constructor.
	 pack();
         try
         {
            jbInit();
	 ...
	 //treeClusters = new TreeDialog(frameMain, Groups, ID, flag, true);
	 //treeClusters.setLocation(691,440);
	 //treeClusters.setVisible(true);
	 ...
	 reSolveDialog.getContentPane().add(pnlTree, BorderLayout.CENTER);
         reSolveDialog.getContentPane().add(okPanel,BorderLayout.SOUTH);
         okPanel.add(okBtn);
         okPanel.add(toolbarStatus, BorderLayout.SOUTH);
         reSolveDialog.show();
      }


      
      void btnRepick_actionPerformed(ActionEvent e)
	 ...
	 reSolveID = -1;
	 rePickID = -1;
         rePickDialog = new JDialog(frameMain,"Re-Pick",true);

         //final JTextField cluster = new JTextField();
         JTextArea text = new JTextArea("Enter the cluster ID of the cluster you want to re-pick");
         text.setBackground(Color.lightGray);
         JPanel okPanel = new JPanel();
	 //JPanel tree = new JPanel();  /// panel for the tree
	 JButton okPickBtn = new JButton();
	 //border1 = BorderFactory.createBevelBorder(BevelBorder.LOWERED,Color.white,Color.white,new Color(142, 142, 142),new Color(99, 99, 99));
	 ...
	 sbStatus.setPreferredSize(new Dimension(20, 15));
         ...
         toolbarStatus.add(sbStatus);
         sbStatus.addPanel("No Cluster Selected",500,sbStatus.ALIGN_CENTER);
	 pnlTree = new SKScrollableTree(this,frameMain,Groups,null,ID,flag,1); // changed
	 pack();
         try
         {
            jbInit();
	 ...
	 //treeClusters.setVisible(true);
         ...
         okPickBtn.setText("OK");
         okPickBtn.addActionListener(
                                 new java.awt.event.ActionListener()
                                 {
                                    ...
                                    {
                                       //reSolveID = java.lang.Integer.parseInt(cluster.getText());
				      //System.out.println( "Coming to the button nclicked part!!!! ");
				       if(rePickID != -1)
					{
						System.out.println(" The repicked Cluster is:: " + rePickID);
						reSolveFlag = -1;
                                       		rePickDialog.hide();
					}
                                    }
         ...
         //reSolveDialog.setSize(250,100);
	 rePickDialog.setSize(500,450);
         rePickDialog.setLocation(691,420);
         rePickDialog.getContentPane().add(text,BorderLayout.NORTH);
         //reSolveDialog.getContentPane().add(cluster,BorderLayout.CENTER);
	 rePickDialog.getContentPane().add(pnlTree, BorderLayout.CENTER);
         rePickDialog.getContentPane().add(okPanel,BorderLayout.SOUTH);
         okPanel.add(okPickBtn);
         okPanel.add(toolbarStatus, BorderLayout.SOUTH);
         rePickDialog.show();
      }


      // These functions added to support the resolve dialogue
      public void reDrawTree()
      {
            ...
            switch(groupName.charAt(0))
            {
               case 'G':
                  if(groupName.charAt(1)=='r')
		            ...
		            groupID = Integer.parseInt(groupName.substring(1));
			    if(reSolveFlag == 1)
			    	reSolveID = groupID;
			    else
			    if(reSolveFlag == 2)
			    	rePickID = groupID;   // Check the resolve flag and correspomdingly display the resolved repicked value
        			break;
			}
               case 'C':
	       {
	    ...
	    // code added for the status bar
	       sbStatus.updatePanelText("Cluster To resolve :" + reSolveID,0);
	       this.repaint();
		...
		int[] newFlag = new int[finalArray.size()+1];
	       ...
	       pnlTree.Tree = finalArray;
	       pnlTree.Root = root;
	       pnlTree.ID = root.ID;
	       pnlTree.flag = newFlag;
	       pnlTree.drawTree();
...
//                }
   ...
   /////////////////////////////////////////////////////
      ...
      {
         int w=pnlShapes.getWidth();
         int h=pnlShapes.getHeight();
         for(int i=0; i<pnlShapes.getComponentCount(); i++)
         {
            SKBaseShape sh= (SKBaseShape) pnlShapes.getComponent(i);
	    if(sh instanceof SKPointSubShape)
		sh=((SKPointSubShape)sh).parentShape;
            if((sh.getX()+15>0) && (sh.getX()+15<w))
               if((sh.getY()+55<h) && (sh.getY()+55>0))
                  g.drawString(sh.Name.charAt(0)+""+sh.ID, sh.getX()+15, sh.getY()+55);}
      }
         ...
         if(count<=1)
         {
            ...
            //         for (int j=1;j<((SKBaseShape)pnlShapes.getComponent(i)).getNumSubShapes()+1; j++)
            {
               ...
               {SKBaseShape sh1 = frameMain.allshapes.findByID(id);
                  //System.out.println("count = "+count+" "+sh1);
      ...
      public boolean checkRotation(SKBaseShape fix1, SKShapeArray nonSolvingShapes, SKShapeArray solvingShapes)
      {
         ...
         int[] savedX = new int[solvingShapes.size()];
         int[] savedY = new int[solvingShapes.size()];
         ...
         double rad=java.lang.Math.PI*2;
      
         for(int i=0; i<solvingShapes.size(); i++)
         {
            savedX[i] = solvingShapes.get(i).getShapeX();
            savedY[i] = solvingShapes.get(i).getShapeY();
         }	
      
         //while(!outside && ((rad+java.lang.Math.PI/36)<=java.lang.Math.PI*2))
         while(!outside && ((rad-java.lang.Math.PI/36)>=0))
         {
            outside = true;
            //System.out.println("rad= "+rad*180/java.lang.Math.PI);
            for(int i=0; i<nonSolvingShapes.size(); i++)
            {
               px = nonSolvingShapes.get(i).getShapeX();
               py = nonSolvingShapes.get(i).getShapeY();
               int vec = ((py-p1y)*(p2x-p1x))-((px-p1x)*(p2y-p1y));
               outside = outside && (((py-p1y)*(p2x-p1x))-((px-p1x)*(p2y-p1y)) > 0);
               //System.out.println("outside = "+vec+" for "+nonSolvingShapes.get(i));
            }
            if(!outside)
            {
               float r = distance(p1x,p1y,p2x,p2y);
               rad = rad-(java.lang.Math.PI/36);
               double angle = java.lang.Math.PI/36;
                     ...
                     //for(int k=0; k<frameMain.allshapes.size(); k++)
                        //for(int j=1; j<frameMain.allshapes.get(k).getNumSubShapes()+1; j++)
                           ...
                           //{
                              ...
                              //System.out.println("b4 "+rad+" "+sh+" "+sh.getX()+" "+sh.getY());
                     sol.setShapePoint(new Point(java.lang.Math.round((float)dst[0]),java.lang.Math.round((float)dst[1])));
                     //System.out.println(sol+"b4 "+rad+" "+px+" "+py+" after "+dst[0]+" "+dst[1]);
                           //}
                  }
               }
      ...
      public void convexHull(SKBaseShape fix1, SKShapeArray Shapes, int points[])
      {
         ...
         while((!outside) && (l<temp.size()))
         {
            ...
            l++;
            System.out.println("convex hull "+fix1+" "+tempSh+" "+outside+" "+p1x+" "+p1y+" "+p2x+" "+p2y);
         }
         points[0] = p1x;
         points[1] = p1y;
         points[2] = p2x;
      ...
      public void cloneConstraints(SKShapeArray arrShapes)
      {
	 SKConstraintArray arrCon = new SKConstraintArray(1);
	 for(int i =0; i<arrShapes.size(); i++)
	 {
	   SKBaseShape sh = arrShapes.get(i);
	   SKBaseShape mainSh = null;
	   for(int j=0; j<frameMain.allshapes.size(); j++)
	   {
		if(sh.ID == frameMain.allshapes.get(j).ID)
		{  mainSh = frameMain.allshapes.get(j);
		   break;
		}
	   }	   
	   if(mainSh != null)
	   {
	     ...
	     {
		SKBaseConstraint mainCon = mainSh.ConstraintList.get(j);
		...
		{
		  arrCon.add(frameMain.createCloneConstraint(mainCon,arrShapes));
		}
	     }
	   }
	 }
