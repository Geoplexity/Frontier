...
public:
  ...
  // Returns values a concatenated string
  char* Disp_Val()
  {
    //cout << type1 << "," << part1 << "," << type2 ;
    //cout << "," << part2 << "," << const_type << "\n";
...
public:
  ...
  */
  ...
  //Stores information of Node into Array
  void copyNodeIntoArray(int Array[], int index, int nodeNo, int pIndex);
...
public:
  ...
  void copyTreeIntoArray()
  {
    ...
    Node *nd = NULL;
    int index=0, parIndex=0, nodeNo=0;
    //Q.push_back(Block[0]);
    ...
    int count=1;
    nd=Block[0];
    suppQ.push_back(nd->child_count);
    while (count <= nd->child_count)
      {
	...
	cout << "strlen is " << strlen(nd->label) << endl;
	name[index-2]= new char[strlen(nd->label)+1];
	strcpy(name[index-2],nd->label);
	
	if (nd->parent !=NULL)
	  {
	    cout << "2. strlen is " << strlen(nd->parent->label) << endl;
	    name[index-1]= new char[strlen(nd->parent->label)+1];
	    strcpy(name[index-1],nd->parent->label);
	  }
    ...
    array[0]=array[1]=0;
    index+=2;
    Block[0]->copyNodeIntoArray(array,index,1,0);
    */
	...
	if (array[i]!=-2)
	  {
	    cout << "index=" << i << " " << array[i] << " " << name[i] << endl;
	  } 
      }
    cout << "The array\n";
  }
...
void Node::copyNodeIntoArray(int array[], int index, int nodeNo, int pIndex)
{
  ...
  for (int i=1; i<=child_count; i++)
    {
      ptr[i-1]->copyNodeIntoArray(array, index, nodeNo, pIndex); 
    }
}
