 ...
 */
   ...
   public class SKUTUFile
   {
      ...
      public static SKScrollableTree pnlTree;
      //public static int ID,w,h;
      public static SKScrollableTree panel= null;
      public  static JStatusBar sbStatus = new JStatusBar();
      public  static JToolBar toolbarStatus = new JToolBar();
	// end aditee


   /**
   *Method that reads the arrays of solutions sent by the UTU and redraws the solved sketch using the new coordinates.
   *Also dssssisplays the DR-Dag on a separate window
   */
...
//          sbStatus.addPanel("No Cluster Selected",500,sbStatus.ALIGN_CENTER);
		SKMainFrame tempFrame = frameMain;
		SKGTNArray tempGrp = new SKGTNArray(1);
		dag.copyArrayTo(tempGrp);
		int tempID = actCluster;
		int[] tempFlag = fin;

		TreeFrame dummy = new TreeFrame(tempFrame, tempGrp, tempID, tempFlag);
		dummy.hide();
	 	pnlTree = new SKScrollableTree(dummy,tempFrame, tempGrp, null, tempID, tempFlag,0);
                  ...
                  if(doneShapes.indexOf(sh) == -1)
                  {
                     ...
                     if (sh instanceof SKPointShape)
                     {
                        ...
                        else 
                        {
                           ...
                           if(threeD)
			   	{
					//((SKPointShape3d)sh3d).name.setString("P"+sh.ID);
...
//          sbStatus.addPanel("No Cluster Selected",500,sbStatus.ALIGN_CENTER);
	    ...
	    reSol = true;
            ...
            {
		System.out.println(" Coming to the iteration " + countIter++ + "  " + indexInt +  " " + dataInt[indexInt] + "  " + startBifurChoice + " " + reSolveID);
	       ...
	       //if((rePickID != -1 && (clusterArray.findByID(dataInt[indexInt])==null)) || (rePickID == -1))
	       {
			...
			int tempRePickID = -1;
               ...
               //Show the dialog
               		if(threeD)
               		{
                  		dlg = new SKBifurFrame(frame,"Bifurcations",true,dag,actCluster,fin); // changed for 3D
...
// 							System.out.println(" Parents..... " + child.parents.get(pa).ID);
						...
						reSol=false;
                  		...
                  		{
                       			if(threeD) // Changed the part for 3D
					{
						...
						if(dataInt[savedIndex] == reSolveID)
			 			...
			 			clusterArray.findByID(dataInt[savedIndex]).selectBifur = dlg.currBifurcation;
					}
   ...
   //	  	       System.out.println("111 Choice"+dlg2d.currBifurcation);
      ...
      //// This is added to support the functionality of the reSolveDialogue in the loop
      ...
      public static void reDrawTree()
      {
         if(mouseOverItem != null)
         {
	       ...
	       SKGTNArray finalArray = new SKGTNArray(1);
               SKGroupTreeNode node = dag.findByID(groupID);
		  ...
		  if(finalArray.indexOf(child) == -1)
	       ...
	       {
	    	   node = dag.findByID(childArray.get(0).ID);
		...
		for(int i=0; i<newFlag.length; i++) // The value of this fin flag is important... it tells something ... i think fin = 1 means solution got!
               		newFlag[i]=1;

// 	       for(int i = 0; i<finalArray.size(); i++)
//                   {
//                      SKGroupTreeNode g = dag.findByID(finalArray.get(i).ID);
//                      //newFlag[i+1] = flagTree[dag.indexOf(g)];
//                   }
      ...
      public static void drawOriginalTree()
      {
      	 int[] newFlag = new int[dag.size()+1];
		for(int i=0; i<newFlag.length; i++) // The value of this fin flag is important... it tells something ... i think fin = 1 means solution got!
               		newFlag[i]=1;

         pnlTree.Tree=dag;
         pnlTree.Root=null;
         pnlTree.ID=actCluster;
         pnlTree.flag=newFlag;
         pnlTree.drawTree();
	 reSolveID = -1;
	 rePickID = -1;
