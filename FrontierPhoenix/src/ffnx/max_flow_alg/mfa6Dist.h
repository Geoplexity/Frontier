...
namespace ffnx::mfa {
    ...
    int pushOutside(Cluster &C, Graph &F, Graph &graph0, ostream &file1, ostream &file2, StaticVariables& staticVariables) {
        int i, j, a, K, nFront;
        ...
        for (i = 1; i <= nFront; i++)    // for every frontier vertices
        {
            ...
            for (v2Name = 1; v2Name <= staticVariables.singleVertex; v2Name++) {
        ...
        for (i = 1; i <= nFront; i++)    // for every frontier vertices
        {
            ...
            for (v2Name = 1; v2Name < staticVariables.nextVerName; v2Name++) {
    ...
    Cluster *distributeVert(Vertex &v1, Graph &F, Graph &graph0, ostream &file1, ostream &file2, StaticVariables& staticVariables) {
                ...
                if (v2.isFrozen())   // the other end in F is frozen
                {
                    ...
                    if (k > 0) {
                        if (F.numLabeled() > F.returnDimen()) {
                            ...
                            if (extension(F, graph0, file1, file2, staticVariables) == 1) {
                                newTree = getCluster(F, graph0, staticVariables);
    ...
    Cluster &distributeCl
            (Cluster &C, Graph &F, Graph &graph0, ostream &file1, ostream &file2, StaticVariables& staticVariables) {
        Cluster *newTree;
        int ClusterDepth, v1Name, k;
        Vertex v1;

        v1Name = C.returnCore().returnName();
        if (v1Name > 0 && v1Name <= staticVariables.singleVertex) // distributing single vertex
        {
            //out<<"in distributeCl 1"<<endl;
            v1 = C.returnCore();
            newTree = distributeVert(v1, F, graph0, file1, file2, staticVariables);
            //newTree->output(out);
        ...
        } else  // distributing a cluster
        {
            ...
            k = pushOutside(C, F, graph0, file1, file2, staticVariables);
            if (k > 0) {
                newTree = getCluster(F, graph0, staticVariables);
                return *newTree;
            }
