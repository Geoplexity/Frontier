...
namespace ffnx::mfa {
    ...
    Cluster &popMinDepth(List<Cluster> &DR_Trees, StaticVariables& staticVariables)
    {
       int i,pos,len,depth,coreName,depthMin=staticVariables.nextVerName,nameOfMin=staticVariables.nextVerName;
    ...
    */
    ...
    List<Cluster> &distributeForest(List<Cluster> &DRtrees, Graph &F,
      Graph &graph0, ostream &file1,ostream &file2, int groupID, int indentation, StaticVariables& staticVariables);

    Cluster &distributeTree(Cluster &aDRtree, Graph &F,
      Graph &graph0, ostream &file1, ostream &file2, int indentation, StaticVariables& staticVariables)
    {
       ...
       distriedChild=distributeForest(Children,F,graph0,file1,file2, aDRtree.returnGroup(),indentation, staticVariables);
    ...
    void implicitChild(Cluster &newTree, List<int> &nameList,
                          List<Cluster> &forest, Graph &F, Graph &graph0, StaticVariables& staticVariables)
    {
       ...
       for(i=1;i<=numOfTree;i++)
       {
          ...
          if(aChild<=newTree)
          {
              ...
              temp=getCluster(F, graph0, staticVariables);
    ...
    void addChild(Cluster &newTree, List<Cluster> &popedTrees,
                   List<Cluster> &DR_Trees, Graph &F, Graph &graph0, StaticVariables& staticVariables)
    {
       ...
       implicitChild(newTree, nameList, DR_Trees, F, graph0, staticVariables);

       // look implicit child in popedTrees
       implicitChild(newTree, nameList, popedTrees, F, graph0, staticVariables);
      ...
      Graph &graph0, ostream &file1, ostream &file2, int groupID, int indentation, StaticVariables& staticVariables)
    {
       ...
       {
          aTree = DR_Trees.pop();
          distried=distributeTree(aTree, F, graph0, file1,file2, indentation, staticVariables);
          if(distried.empty())
             delete &distried;
          else
       ...
       while(DR_Trees.returnLen()>0)
       {
          file1<<"---------------------------------------------"<<endl;
          file2<<"---------------------------------------------"<<endl;
          if(extension(F, graph0, file1, file2, staticVariables))
          {
             newTree=*getCluster(F, graph0, staticVariables);
          }
          else
          {
             aTree=popMinDepth(DR_Trees, staticVariables);  // item is deleted in DR_Trees
             ...
             F.output(file2);
             newTree=distributeCl(aTree, F, graph0, file1, file2, staticVariables);
          }

          if(!newTree.empty()) {
              file2<<"got new Cluster"<<endl;
              F.output(file2);
              addChild(newTree, popedTrees, DR_Trees, F, graph0, staticVariables);
    ...
    void getTrivial(List<Cluster> &DRTrees, Graph &graph0, StaticVariables& staticVariables)
    {
       ...
       for(i=1; i<=numDRTree; i++)
       {
          ...
          if(vName1>0&&vName1<=staticVariables.singleVertex)
                tempTrees.append(aTree1);
          else if(vName1>staticVariables.singleVertex)
